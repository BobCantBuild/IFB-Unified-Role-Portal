...
// api/news.js  (or wherever your /api/news route lives)
...
const EXCLUDE_KEYWORDS = [
  ...
  'quarterly result', 'dividend', 'market cap', 'earnings',
  'investor', 'trading', 'intraday', 'target price',
];
...
module.exports = async (req, res) => {
  try {
    ...
    const allItems = [];
    const seenTitles = new Set();
    const cutoff = Date.now() - 48 * 60 * 60 * 1000; // last 48 hours only
    ...
    for (const result of results) {
      ...
      for (const item of result.value.items) {
        ...
        if (pubDate < cutoff)       continue; // too old
        if (isExcluded(title))      continue; // financial news
        if (seenTitles.has(key))    continue; // duplicate
