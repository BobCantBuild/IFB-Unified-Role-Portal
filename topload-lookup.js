/* ══════════════════════════════════════════════════════
   topload-lookup.js
   Top Load search bar — standalone, independent file.
   Requires: model-lookup.js loaded first (shares overlay via window.MLOverlay)
══════════════════════════════════════════════════════ */
(function () {

  /* ─────────────────────────────────────────
     TL program descriptions
     Exposed globally so model-lookup.js overlay
     can pick the right descMap when a TL model opens.
  ───────────────────────────────────────── */
  window._TL_DESC = {
  ...
  if (!tlInput) {
    console.warn('[TL Search] #tlSearchInput not found in DOM.');
    return;
  }
  ...
  function tlHide() {
    tlSuggest.style.display = 'none';
    tlList = [];
  }

  /* ─────────────────────────────────────────
     API — debounced autocomplete
  ───────────────────────────────────────── */
  async function tlFetch(q) {
    ...
    try {
      ...
      console.error('[TL Search] fetch error:', err);
      tlHide();
    }
  }

  /* ─────────────────────────────────────────
     API — full search → open overlay
  ───────────────────────────────────────── */
  async function tlSearch() {
    ...
    // Grab overlay helpers from model-lookup.js
    ...
    try {
      ...
      if (json.results?.length) {
        // Open shared overlay via model-lookup.js bridge
        if (window.MLOverlay?.open) {
          window.MLOverlay.open(json.results[0]);
        } else {
          console.error('[TL Search] window.MLOverlay not ready. Is model-lookup.js loaded first?');
        }
      } else {
        // Show "not found" state in overlay
      ...
      console.error('[TL Search] search error:', err);
      const overlay = document.getElementById('modelOverlay');
      const titleEl = document.getElementById('modelOverlayTitle');
      const tabsEl  = document.getElementById('modelTabs');
      const bodyEl  = document.getElementById('modelTabBody');
  ...
  /* ─────────────────────────────────────────
     Events
  ───────────────────────────────────────── */
  tlSuggest.addEventListener('click', e => {
  ...
  document.addEventListener('click', e => {
    if (!tlInput.contains(e.target) && !tlSuggest.contains(e.target)) tlHide();
  });

  console.log('[TL Search] topload-lookup.js ready ✅');
